...
final class Flag implements JsonSerializable {
	...
	protected function get_published_flags(): array {
		...
		return $this->sanitize_flags( $published_flags );
	}
	...
	protected function get_user_flags( int $user_id = 0 ): array {
		...
		return $this->sanitize_flags( $user_flags );
	}
	 ...
	 * @return array<Flag>
	 */
	protected function sanitize_flags( mixed $flags ): array {
		if ( ! is_array( $flags ) ) { 
			return array();
		}
